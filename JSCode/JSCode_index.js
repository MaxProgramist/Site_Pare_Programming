...
async function JoinRoom() {
    ...
    if (joinRoomCodeValue.length < 4) return PopUpWindowOfError("Incorrect code type (must be 4 numbers)");
    if (nicknameValue.length < 1) return PopUpWindowOfError("Incorrect nickname type (at least 1 symbol)");

    for (let i = 0; i < joinRoomCodeValue.length; i++) {
        let charCode = joinRoomCodeValue.charCodeAt(i);
        if (charCode < 48 || charCode > 57) return PopUpWindowOfError("Incorrect code type (must be 4 numbers)");
    }
...
async function MakeRoom() {
    ...
    if (makeRoomCodeValue.length < 4) return PopUpWindowOfError("Incorrect code type (must be 4 numbers)");
